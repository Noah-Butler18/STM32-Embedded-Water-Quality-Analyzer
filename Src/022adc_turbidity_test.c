 ...
 * NOTE: set switch on sensor to "A"
 *
 * Hardware connections from stm32 to turbidity sensor:
 * 		1)V (Vin) - 5V
 * 		2)G (gnd) - common gnd
 *
 * Hardware connections for logic level shifter and stm32:
 * 		1)HV - 5V
 * 		2)LV - 3V
 * 		3)GND (LV side only) - common GND
 *
 * 		4)HV1 - S
 * 		5)LV1 - PA1
 *
 * 		Objective: test voltage readings from turbidity using different water sources with varying levels of cloudiness
 * 					Make formula to convert voltage to turbidity in units of NTUs and ppm
 */
