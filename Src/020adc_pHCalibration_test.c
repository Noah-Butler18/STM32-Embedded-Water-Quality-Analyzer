 ...
 * 2. Hook up the output of the ph sensor to PA1 on STM32 through the logic level converter on the breadboard
 * 3. Configure TIM2 timer to generate an interrupt every so often
 * 4. When TIM2 timer interrupt is generated, read ADC value and store in user variable
 *
